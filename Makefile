...
.PHONY: help venv install test run build dist pkg version release clean clean-all
...
pkg: build ## Construye el instalador .pkg
	@$(ACTIVATE) && VERSION=$$(python -c "from app.version import __version__; print(__version__)") && \
	rm -rf pkg-root && \
	mkdir -p pkg-root/Applications && \
	cp -R "$(DIST_APP)" "pkg-root/Applications/" && \
	pkgbuild \
		--root pkg-root \
		--identifier com.fiftyflowers.imagetransformlite \
		--version "$$VERSION" \
		--install-location / \
		"dist/Image-Transform-Lite-$$VERSION-arm64.pkg" && \
	rm -rf pkg-root && \
	echo "Pkg listo: dist/Image-Transform-Lite-$$VERSION-arm64.pkg"
